...
public:
    ...
    typedef Eigen::Matrix<value_type, Eigen::Dynamic, Eigen::Dynamic> pmatrix_type;
    ...
    void do_step( System system , const state_type &x , time_type t , state_type &xout , time_type dt , state_type &xerr )
    {
        ...
        matrix_type m_identity( n, n );
        m_identity.setIdentity();
        m_jac.m_v = -m_jac.m_v + 1.0 / m_coef.gamma / dt * m_identity;

        // Eigen::BiCGSTAB<matrix_type> solver(m_jac.m_v);
        Eigen::PardisoLU<matrix_type> solver(m_jac.m_v);

        m_g1.m_v = m_dxdt.m_v;
        m_g1.m_v = solver.solve(m_g1.m_v).eval();


        m_xtmp.m_v = x + m_coef.a21 * m_g1.m_v;
        deriv_func( m_xtmp.m_v , m_dxdtnew.m_v , t + m_coef.c2 * dt );
        m_g2.m_v = m_dxdtnew.m_v + m_coef.c21 * m_g1.m_v / dt;
        m_g2.m_v = solver.solve(m_g2.m_v).eval();


        m_xtmp.m_v = x + m_coef.a31 * m_g1.m_v + m_coef.a32 * m_g2.m_v;
        deriv_func( m_xtmp.m_v , m_dxdtnew.m_v , t + m_coef.c3 * dt );
        m_g3.m_v = m_dxdtnew.m_v  + ( m_coef.c31 * m_g1.m_v + m_coef.c32 * m_g2.m_v ) / dt;
        m_g3.m_v = solver.solve(m_g3.m_v).eval();


        m_xtmp.m_v = x + m_coef.a41 * m_g1.m_v + m_coef.a42 * m_g2.m_v + m_coef.a43 * m_g3.m_v;
        deriv_func( m_xtmp.m_v , m_dxdtnew.m_v , t + m_coef.c4 * dt );
        m_g4.m_v = m_dxdtnew.m_v + ( m_coef.c41 * m_g1.m_v + m_coef.c42 * m_g2.m_v + m_coef.c43 * m_g3.m_v ) / dt;
        m_g4.m_v = solver.solve(m_g4.m_v).eval();


        m_xtmp.m_v = x + m_coef.a51 * m_g1.m_v + m_coef.a52 * m_g2.m_v + m_coef.a53 * m_g3.m_v + m_coef.a54 * m_g4.m_v;
        deriv_func( m_xtmp.m_v , m_dxdtnew.m_v , t + dt );
        m_g5.m_v = m_dxdtnew.m_v + ( m_coef.c51 * m_g1.m_v + m_coef.c52 * m_g2.m_v + m_coef.c53 * m_g3.m_v + m_coef.c54 * m_g4.m_v ) / dt;
        m_g5.m_v = solver.solve(m_g5.m_v).eval();

        m_xtmp.m_v += m_g5.m_v;
        deriv_func( m_xtmp.m_v , m_dxdtnew.m_v , t + dt );
        xerr = m_dxdtnew.m_v + ( m_coef.c61 * m_g1.m_v + m_coef.c62 * m_g2.m_v + m_coef.c63 * m_g3.m_v + m_coef.c64 * m_g4.m_v + m_coef.c65 * m_g5.m_v ) / dt;
        xerr = solver.solve(xerr).eval();

        xout = m_xtmp.m_v + xerr;
        // auto end_time = std::chrono::high_resolution_clock::now();
        // std::cout << std::fixed << std::setprecision(3)
        //     << "Step runtime: " << std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time).count() / 1000.0
        //     << " s\n";
    }
    ...
    {
        m_cont3.m_v = m_coef.d21 * m_g1.m_v + m_coef.d22 * m_g2.m_v + m_coef.d23 * m_g3.m_v + m_coef.d24 * m_g4.m_v + m_coef.d25 * m_g5.m_v;
        m_cont4.m_v = m_coef.d31 * m_g1.m_v + m_coef.d32 * m_g2.m_v + m_coef.d33 * m_g3.m_v + m_coef.d34 * m_g4.m_v + m_coef.d35 * m_g5.m_v;
    }
            ...
            const state_type &x_new , time_type t_new )
    {
        ...
        time_type s1 = 1.0 - s;
        x = x_old * s1 + s * ( x_new + s1 * ( m_cont3.m_v + s * m_cont4.m_v ) );
    }
